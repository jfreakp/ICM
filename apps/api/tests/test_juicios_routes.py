...
async def test_fecha_minima_includes_seeded_row(client, db_session):
    headers = await _auth_headers(client, db_session)
    fecha_anterior = await _fecha_minima_actual(client, headers, "/api/reportes/juicios/fecha-minima")
    await _seed_juicios(
        db_session, [_row("TEST-JUI-FMIN-001", fecha_anterior, identificacion="TEST-JUI-CED-fmin1")]
    )
...
async def test_fecha_minima_ignores_soft_deleted_rows(client, db_session):
    ...
    fecha_anterior = await _fecha_minima_actual(client, headers, "/api/reportes/juicios/fecha-minima")
    await _seed_juicios(
        db_session, [_row("TEST-JUI-FMIN-101", fecha_anterior, identificacion="TEST-JUI-CED-fmin2")]
    )
    await db_session.execute(
